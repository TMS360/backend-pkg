...
type TeamsServiceClient interface {
	...
	// DEV-2240: who held a truck or trailer over a DATE RANGE. Every other crew
	// read on this service answers "right now" / "on this day" for one instant;
	// an asset charge (truck payment, trailer rent) is split between the drivers
	// who held the asset on each day of a period, so the caller needs the whole
	// period in one answer — polling the instant read day by day would still miss
	// an intra-day swap.
	//
...
func (c *teamsServiceClient) GetActiveCrewByDriver(ctx context.Context, in *GetActiveCrewByDriverRequest, opts ...grpc.CallOption) (*GetActiveCrewByDriverResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(GetActiveCrewByDriverResponse)
	err := c.cc.Invoke(ctx, TeamsService_GetActiveCrewByDriver_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _TeamsService_GetActiveCrewByDriver_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GetActiveCrewByDriverRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(TeamsServiceServer).GetActiveCrewByDriver(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: TeamsService_GetActiveCrewByDriver_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(TeamsServiceServer).GetActiveCrewByDriver(ctx, req.(*GetActiveCrewByDriverRequest))
	}
	return interceptor(ctx, in, info, handler)
}
